...
import { EditSubTodoAction } from "@/lib/actions/todo/subtodo/edit";
...
export function Form({ data }: { data: SubTodoProps }) {
  const initialState = {
    ...
    message: "",
    id: data.id,
    parentTodo: data.todoID,
  };

  const [state, formAction] = useFormState(EditSubTodoAction, initialState);

  useToast({
    message: state.message,
    type: state.type,
    successMessage: "Successfully Edited Todo",
    successRoute: `/todo/${data.todoID}/${data.id}`,
  });
